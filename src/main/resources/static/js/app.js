...
app.controller("CategoryController", function ($scope, $http) {
    ...
    $scope.subscribe = function (categoryId) {
        $http.post('/categories/' + categoryId + '/subscribe/').success(function (data, status, headers, config) {
            for (var i = 0; i < $scope.subscriptions.length; i++) {
    ...
    $scope.getSubscriptions = function () {
        $http.get('/categories/subscriptions').success(function (data, status, headers, config) {
    ...
    $scope.getJournals = function () {
        $http.get('/rest/journals/').success(function (data, status, headers, config) {
            ...
            for (var i = 0; i < journals.length; i++) {
                ...
                journal.readLink = "/rest/journals/" + journal.id + "/view/";
...
app.controller("BrowseController", function ($scope, $http, $filter, $window) {
    ...
    $scope.delete = function (id) {
        $http.delete('/rest/journals/' + id + '/unPublish/').success(function (data, status, headers, config) {
